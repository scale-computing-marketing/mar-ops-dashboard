...
function valueParas(text,color){color=color||DC.body;const base=`<w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:color w:val="${color}"/><w:sz w:val="20"/>`;return String(text==null?'':text).split(/\n/).map(b=>`<w:p><w:pPr><w:spacing w:before="20" w:after="20"/></w:pPr>${runsXml(b,base)}</w:p>`).join('');}
...
function bodyParas(html){
  ...
  if(!html.trim()) return `<w:p><w:pPr><w:spacing w:before="20" w:after="20"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:sz w:val="20"/></w:rPr><w:t xml:space="preserve"></w:t></w:r></w:p>`;
  ...
  return paras.map(arr=>{
    const inner=arr.map(r=>{
      ...
      const props=`<w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:color w:val="${DC.body}"/><w:sz w:val="20"/>`+(r.b?'<w:b/>':'')+(r.i?'<w:i/>':'')+(r.u?'<w:u w:val="single"/>':'');
...
function drow(label,value,o){o=o||{};const vc=o.valueColor||DC.body;const lbl=`<w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:b/><w:color w:val="${DC.navy}"/><w:sz w:val="20"/></w:rPr><w:t xml:space="preserve">${esc(label)}</w:t></w:r>`+(o.required?`<w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:b/><w:color w:val="${DC.red}"/><w:sz w:val="20"/></w:rPr><w:t xml:space="preserve">  *</w:t></w:r>`:'');
...
function dheading(text,size,before,after,border){return`<w:p><w:pPr>${border?`<w:pBdr><w:bottom w:val="single" w:sz="6" w:space="6" w:color="${DC.border}"/></w:pBdr>`:''}<w:spacing w:before="${before}" w:after="${after}"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:b/><w:color w:val="${DC.navy}"/><w:sz w:val="${size}"/></w:rPr><w:t xml:space="preserve">${esc(text)}</w:t></w:r></w:p>`;}
function titleBlock(camp){const q=camp.quarterLabel&&camp.quarterLabel!=='N/A'?camp.quarterLabel:'';const y=camp.yearLabel&&camp.yearLabel!=='N/A'?camp.yearLabel:'';
  ...
  const eyebrow=`<w:p><w:pPr><w:spacing w:before="0" w:after="50"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:b/><w:color w:val="${DC.red}"/><w:sz w:val="18"/><w:spacing w:val="60"/></w:rPr><w:t xml:space="preserve">CAMPAIGN BUILD</w:t></w:r></w:p>`;
  const title=`<w:p><w:pPr><w:spacing w:before="0" w:after="60"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:b/><w:color w:val="${DC.navy}"/><w:sz w:val="46"/></w:rPr><w:t xml:space="preserve">${esc(camp.name||'Untitled Campaign')}</w:t></w:r></w:p>`;
  const sub=`<w:p><w:pPr><w:pBdr><w:bottom w:val="single" w:sz="18" w:space="8" w:color="${DC.red}"/></w:pBdr><w:spacing w:before="0" w:after="260"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:color w:val="${DC.grey}"/><w:sz w:val="20"/></w:rPr><w:t xml:space="preserve">${esc(meta)}</w:t></w:r></w:p>`;
...
function buildDocx(data,logo){const body=buildDocBody(data);
  ...
  let headerInner;
  if(hasLogo){const cx=1554480;const cy=Math.round(cx*logo.h/logo.w);
...
function computeEmail(i){
  ...
  const nm=c.name||'Campaign name';
  const num=e.number||String(i+1);
  const pardotName=prefix+nm+(e.hasOrder?(' | Email '+num):'');
  const campaignTag=matched?reg.tag:(ever?('campaign-'+(slug(c.name)||'name')):('campaign-'+(slug(c.name)||'name')+'-q'+(c.quarter||'?')+'-'+(c.year||'yyyy')));
...
function updateCardOutput(i){
  ...
  if(tw)tw.innerHTML=r.tags.map((x)=>`<span class="cb-tag${x.req?' req':''}"><span class="src ${x.src==='reg'?'reg':''}">${x.src==='reg'?'reg':'doc'}</span>${x.t}</span>`).join('');
